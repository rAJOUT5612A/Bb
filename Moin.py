...
async def start(update: Update, context: CallbackContext):
    ...
    if not await is_user_allowed(user_id):
        await context.bot.send_message(
            chat_id=chat_id, 
            text="*𝗕𝗨𝗬 𝗞𝗔𝗥 𝗙𝗥𝗘𝗘 𝗠𝗘 𝗞𝗨𝗖𝗛 𝗡𝗔𝗛𝗜 𝗠𝗜𝗟𝗧𝗔 𝗖𝗢𝗡𝗧𝗔𝗖𝗧 @RAJPUTDDOS",
            parse_mode='Markdown'
        )
        return
...
async def add_user(update: Update, context: CallbackContext):
    ...
    if user_id != ADMIN_USER_ID:
        await context.bot.send_message(
            chat_id=update.effective_chat.id, 
            text="*💀 𝗔𝗕𝗘 𝗡𝗢𝗢𝗕 𝗢𝗪𝗡𝗘𝗥 𝗛𝗜 𝗔𝗗𝗗 𝗞𝗔𝗥 𝗦𝗔𝗞𝗧𝗔 𝗛𝗔𝗜🤬 ⚡ 𝙎𝘾𝙍𝙄𝙋𝙏 𝙆𝙃𝘼𝙍𝙄𝘿 𝘼𝙐𝙍 𝙆𝙃𝙐𝘿 𝙆𝘼 𝘽𝙊𝙏 𝘽𝘼𝙉𝘼 𝙉𝘼𝙃𝙄 𝙏𝙊𝙃 𝘽𝙃𝘼𝙂 𝙅𝘼 🇧 🇺 🇾 ~ @RAJPUTDDOS 𝙋𝙍𝙄𝘾𝙀 - 800 𝘼𝘽 𝙉𝙄𝙆𝘼𝙇",
            parse_mode='Markdown'
        )
        return
